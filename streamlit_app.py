...
def main():
    ...
    with st.spinner("Summarizing and Chatting..."):
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = {executor.submit(fetch_content, article_url): article_url for article_url in article_urls_list}
            for i, (article_url, future) in enumerate(zip(article_urls_list, concurrent.futures.as_completed(futures)),
                                                      start=1):
                try:
                    content = future.result()
                    if content:
                        display_article_summary(i, content)
                        process_user_interaction(i, content, loading_text)
